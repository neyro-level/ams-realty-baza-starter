...
- Master plan: `docs/AMS_MASTER_PLAN №4.md` (`Plan ID: AMS-REALTBASE-CORE-ALIGN`, `Version: v1`, `Status: APPROVED`). Закрытый corrections: `docs/AMS_MASTER_PLAN №3.md`. Закрытый hardening: `docs/AMS_MASTER_PLAN №2.md`. Исторический v1.0: `AMS_PROJECT_ARCHITECTURE_v1.0.md`.
...
3. Релевантный раздел `docs/AMS_MASTER_PLAN №4.md`.
...
- В активной оркестрации плана `AMS-REALTBASE-CORE-ALIGN` после команды владельца закрыть весь план: WORK → commit/push → SourceCraft PR → MERGE_AFTER_GATE в `main` → удалить epic-ветку → следующая READY task. GitHub не трогать. Production в этот план не входит. Независимый Task Manager Code Reviewer только по явному триггеру владельца.
